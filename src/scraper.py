...
class RobloxScraper:
    ...
    def __init__(self, message_cb: MessageCallback | None = None, *, max_concurrency: int = 1000) -> None:
            ...
            try:
                async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                    if resp.status in (429, 500, 502, 503, 504):
                        backoff = min(1.5 ** attempt + random.random() * 0.5, 3.0)
                        self._say(f"Backoff {resp.status}: sleeping {backoff:.2f}s")
